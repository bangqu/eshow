				...
				<div class="well com">
					...
					<ul id="myTab" class="nav nav-tabs">
						<li class="active"><a data-toggle="tab"
							href="<c:url value='/topic/'/>">话题列表</a>
						</li>
						<li><a data-toggle="tab"
							href="<c:url value='/topic/audit'/>">未审核</a>
						</li>
						<li><a data-toggle="tab"
							href="<c:url value='/topic/mine'/>">我发表的话题</a>
						</li>
						<li><a data-toggle="tab"
							href="<c:url value='/topic/replied'/>">我参与的话题</a>
						</li>
						<li><a data-toggle="tab"
								href="<c:url value='/topic/workmate'/>">周围的话题</a></li>
						<li><a data-toggle="tab" href="<c:url value='/board'/>">话题板块</a>
						</li>
					</ul>
					<div id="mainarea">
						<div id="mainarea_bg">
									...
									<div id="mainUse">
										<div class="avatar">
											<c:if test="${view.topic.user.photo==null}">
												<a
													href="<c:url value="/user/view/${view.topic.user.username}"/>"><img
										...
										<div class="submenu">
											<a
												href="<c:url value='/topic/workmate/${view.topic.user.id}'/>">他的话题</a>
										</div>
										<div class="upPage">
											<c:if test="${view.topic.state==0}">
												<input type="button" class="botton" value="审核通过"
													...
													onclick="javascript:location.href='<c:url value='/topic/topic!state?id=${view.topic.id}&topic.state=1&topic.title=title&topic.content=content'/>'" />
												<input type="button" class="botton_close1" value="审核不通过"
													...
													onclick="javascript:location.href='<c:url value='/topic/topic!state?id=${view.topic.id}&topic.state=2'/>'" />
											</c:if>
										</div>
									</div>
									<div class="info">
										...
										<div class="r">
											<a href="/eshow/topic/edit/${view.topic.id}">修改</a>&nbsp;&nbsp;|&nbsp;&nbsp;
											<a href="<c:url value='/topic/view/${view.topic.id}'/>">回复</a>
										</div>
										<div class="c"></div>
									</div>
									<div class="tips">
							...
							<c:if test="${view.topic.state==1}">
								<s:action name="topic-comment!search" var="topicCommentList"
									executeResult="false">
								...
								<s:iterator value="%{#topicCommentList.topicComments}"
									...
									<div class="commenta" id="comDiv5244">
										...
										<div class="commenta_avatar">
											<a
												href="<c:url value='/topic/workmate/${view.topic.user.id}'/>"><img
