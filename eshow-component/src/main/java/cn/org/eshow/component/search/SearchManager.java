...
public class SearchManager<T> {
    ...
    public Page<T> search(Class object, String keyword, Integer offset, Integer pagesize, String... pros) throws Exception {
        ...
        QueryBuilder qb = fullTextSession.getSearchFactory().buildQueryBuilder().forEntity(object).get();
        org.apache.lucene.search.Query query = qb.keyword().onFields(pros).matching(keyword).createQuery();
    ...
    public List searchAll(Class object, String keyword, String... pros) {
        ...
        try {
            Query luceneQuery = parser.parse("title:" + keyword);
            FullTextQuery fullTextQuery = fullTextSession.createFullTextQuery(luceneQuery, object);
