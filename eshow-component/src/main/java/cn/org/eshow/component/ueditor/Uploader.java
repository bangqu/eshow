package cn.org.eshow.component.ueditor;

import cn.org.eshow.component.upyun.UpYun;
import cn.org.eshow.component.upyun.UpYunConfig;
...
public class Uploader {
	...
	public void upload(String bucketname) throws Exception {
		...
		try {
			...
			while (fii.hasNext()) {
				...
				// 初始化空间
				UpYun upyun = new UpYun(bucketname, UpYunConfig.USERNAME, UpYunConfig.PASSWORD);
				if (!fis.isFormField()) {
					...
					// 上传文件，是否自动创建父级目录（最多10级）
					upyun.writeFile(savePath + "/" + fileName,
							FileUtil.inputStreamToByte(fis.openStream()), true);
