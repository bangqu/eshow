...
public class UserAction extends BaseAction {
    ...
    @Autowired
    private CaptchaManager captcheManager;
    ...
    public String check() {
        ...
        if (old == null) {
            CaptchaQuery captcheQuery = new CaptchaQuery();
            captcheQuery.setCode(captche);
            captcheQuery.setMobile(user.getUsername());
            captcheQuery.setState(CommonVar.CODE_UNUSED);
            Captcha obj = captcheManager.check(captcheQuery);
            if (obj == null) {
                saveMessage("验证码错误");
                return REDIRECT;
            } else {
                obj.setState(CommonVar.CODE_USED);// 已使用
                captcheManager.save(obj);
            }
        } else {
    ...
    public void setCaptcha(String captche) {
    ...
    public void setCaptchaType(String captcheType) {
